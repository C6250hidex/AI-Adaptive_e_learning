...
async function compileCpp(commands, sourcePath, outputPath, options = {}) {
  ...
  return (
    lastUnavailable || {
      ...
      unavailable: true,
      output:
        "C++ compiler not available. Install LLVM, MinGW, or Visual Studio Build Tools.",
    }
  );
}
...
function createApp(options = {}) {
  ...
  app.post(
    ["/chat", "/api/chat"],
    asyncHandler(async (req, res) => {
      if (!process.env.GEMINI_API_KEY && !options.geminiApiKey) {
        return res
          .status(503)
          .send(
            "AI chat service is not configured. Please set GEMINI_API_KEY.",
          );
      }
...
`;
      ...
      try {
        ...
        if (error.message && error.message.includes("API key")) {
          return res
            .status(503)
            .send("AI service is temporarily unavailable. Try again later.");
        }
        throw error;
      }
    }),
    ...
    asyncHandler(async (req, res) => {
      if (!process.env.GEMINI_API_KEY && !options.geminiApiKey) {
        return res
          .status(503)
          .send(
            "AI explain service is not configured. Please set GEMINI_API_KEY.",
          );
      }
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      if (!process.env.GEMINI_API_KEY && !options.geminiApiKey) {
        return res
          .status(503)
          .send("AI fix service is not configured. Please set GEMINI_API_KEY.");
      }
